...
class TestCryptoLocker(Test):
	...
	def test_check_for_currently_detected_strings(self):
		# jk: the following strings are not detected at the moment
	...
	def test_check_for_currently_detected_functions(self):
		...
		#assert self.out_c.has_func( 'function_406b90' ) # cfg fnc detect would find it, if BB was not referenced from wrongly decoded data
		...
		#assert self.out_c.has_func( 'function_40b250' ) # not sure if this should be function, in decfront it was probably found by cfg fnc detect based on tail call -- last instruction in prev fnc is simple jmp, not ret
		assert self.out_c.has_func( 'function_40b2b4' )
		#assert self.out_c.has_func( 'function_40b2d0' ) # same as function_40b250
		...
		#assert self.out_c.has_func( 'function_40ce20' )  # same as function_40b250
		...
		#assert self.out_c.has_func( 'function_40db20' )  # same as function_40b250
	...
	def test_check_std_functions_calls(self):
		...
		assert fnc.calls( 'RegFlushKey' )
		assert fnc.calls( 'ReleaseMutex' )
		fnc = self.out_c.funcs[ '__purecall' ]
		assert fnc.calls( '_purecall' )
		fnc = self.out_c.funcs[ '__except_handler3' ]
		assert fnc.calls( '_except_handler3' )
		fnc = self.out_c.funcs[ '_memcpy_1' ]
		assert fnc.calls( 'memcpy' )
		fnc = self.out_c.funcs[ '_memset' ]
		assert fnc.calls( 'memset' )
