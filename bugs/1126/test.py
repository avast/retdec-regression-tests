...
class Test(Test):
    ...
    def test_statically_linked_functions_are_called(self):
        assert self.out_c.funcs['mainCRTStartup'].calls('__security_init_cookie')
        assert self.out_c.funcs['getParams'].calls('__security_check_cookie')
        assert self.out_c.has_comment_matching(r'.*int32_t __tmainCRTStartup\(void\);')
