...
class TestAll(TestBase):
    ...
    def test_for_string(self):
        ...
        # puts() is used -> no '\n' at the end of the string
        assert self.out_c.has_string_literal_matching( r'Base1 test:(\\n)?' )
...
class TestMsvcDebug(TestBase):
    settings = CriticalTestSettings(
        input='inputs/msvc/derived-multiple-msvc-debug.ex',
        args='-k'
    )
...
class TestMsvcRelease(TestBase):
    settings = CriticalTestSettings(
        input='inputs/msvc/derived-multiple-msvc-release.ex',
        args='-k'
    )
